...
export function PartcilesSnakeCanvas({ children }) {
  ...
  const maxTrailLengthLimit = 200;   // Longitud máxima final del rastro
  ...
  class Particle {
    ...
    draw(ctx) {
      ...
      /*
      // Dibujar el rastro (cuerpo de la serpiente)
      if (this.trail.length > 1) {
        for (let i = 0; i < this.trail.length - 1; i++) {
          // alpha va de 0 (cola) a casi 1 (cabeza)
          const alpha = i / (this.trail.length - 1);
          ctx.beginPath();
          ctx.moveTo(this.trail[i].x, this.trail[i].y);
          ctx.lineTo(this.trail[i + 1].x, this.trail[i + 1].y);
          ctx.strokeStyle = `rgba(255, 255, 255, ${alpha})`;
      ...
      // Dibujar la cabeza (círculo brillante)
      ctx.beginPath();
